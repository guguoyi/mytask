...
class VoteStoreMain extends React.PureComponent {
  ...
  render() {
    const { combilneReducerStore } = this.context;
    const { supNum, oppNum } = combilneReducerStore.getState().vote;
  ...
  componentDidMount() {
    this.unsubscribe = this.context.combilneReducerStore.subscribe(() => {
      this.forceUpdate();
    });
  }
